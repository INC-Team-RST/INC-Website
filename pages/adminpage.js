  ...
  return (
    <div className="bg-[#eaf3fa] w-screen h-screen flex flex-col text-center items-center font-myfont py-10 px-10 ">
      <IoLogOut
        ...
        onClick={logout}
      />
      <div className="text-[#30498f] font-bold text-4xl mb-10">
        {" "}
        Admin at Client Hive{" "}
      </div>
      ...
      {admin?.profession !== "NOT_SELECTED" && (
        <button
          onClick={() => updateAdminService(adminService)}
          className="text-xl p-5 text-[1.2rem] my-6 rounded-2xl bg-[#f59d55]"
        >
          {" "}
          Continue as Admin-profession :{" "}
          <span className="text-[#30498f] font-medium"> {adminService} </span>
        </button>
      )}
    </div>
  );
